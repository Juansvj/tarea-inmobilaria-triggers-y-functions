...
ALTER TABLE contrato
ADD FOREIGN KEY (id_agente) REFERENCES agente(cod_agente);

-- 1. FN_CalcularPrecioSugerido: Calcula el precio sugerido para una propiedad según características. 
DELIMITER //

CREATE FUNCTION FN_CalcularPrecioSugerido(area DECIMAL(10,2))
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
    DECLARE precio DECIMAL(10,2);
    SET precio = area * 2000;
    RETURN precio;
END //
...
DROP FUNCTION IF EXISTS FN_ObtenerPropiedadesCompatibles;

DELIMITER //

CREATE FUNCTION FN_ObtenerPropiedadesCompatibles(precio_max DECIMAL(10,2))
RETURNS INT
DETERMINISTIC
BEGIN
    DECLARE total INT;
...
CREATE FUNCTION FN_CalcularRentabilidadAlquiler(alquiler DECIMAL(10,2), precio DECIMAL(10,2))
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
    RETURN (alquiler * 12 / precio) * 100;
END //

DELIMITER ;

SELECT FN_CalcularRentabilidadAlquiler(1000, 100000);
-- 4. FN_EstimarTiempoVenta: Estima el tiempo probable de venta según tipo y zona. 
DELIMITER //

CREATE FUNCTION FN_EstimarTiempoVenta(tipo VARCHAR(50))
RETURNS INT
DETERMINISTIC
BEGIN
    DECLARE tiempo INT;
...
SELECT FN_EstimarTiempoVenta('Casa');
-- 5. FN_CalcularCuotaHipotecaria: Calcula la cuota hipotecaria estimada para una propiedad.
DELIMITER //

CREATE FUNCTION FN_CalcularCuotaHipotecaria(monto DECIMAL(10,2), tasa DECIMAL(5,2), años INT)
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
    DECLARE cuota DECIMAL(10,2);
    
    SET cuota = (monto * tasa / 100) / 12;
...
SELECT FN_CalcularCuotaHipotecaria(200000, 10, 20);