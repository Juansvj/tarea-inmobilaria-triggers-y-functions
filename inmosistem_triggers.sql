...
ALTER TABLE venta ADD cod_agente INT;
...
BEGIN
    UPDATE propiedad
    SET disponible = 'NO'
    WHERE cod_propiedad = NEW.cod_propiedad;
END//
...
BEGIN
    UPDATE agentes
    SET comision_total = comision_total + (NEW.precio_final * 0.03)
    WHERE cod_agente = NEW.cod_agente;
END//

DELIMITER ;
-- 3. TR_ActualizarEstadisticasVentas: Actualiza estadísticas de ventas por zona y agente.
DROP TRIGGER IF EXISTS TR_CalcularComisionesAgentes;
...
BEGIN
    IF OLD.precio <> NEW.precio THEN
        INSERT INTO historial_precios(cod_propiedad, precio_anterior, precio_nuevo, fecha)
        VALUES (OLD.cod_propiedad, OLD.precio, NEW.precio, NOW());
    END IF;
END//
...
FOR EACH ROW
BEGIN
    DECLARE precio_propiedad DECIMAL(10,2);
    DECLARE presupuesto_cliente DECIMAL(10,2);

    SELECT precio INTO precio_propiedad
    FROM propiedades
    WHERE cod_propiedad = NEW.cod_propiedad;

    SELECT presupuesto INTO presupuesto_cliente
    FROM clientes
    WHERE cod_cliente = NEW.cod_cliente;

    IF precio_propiedad > presupuesto_cliente THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'La propiedad excede el presupuesto del cliente';
    END IF;
END//
